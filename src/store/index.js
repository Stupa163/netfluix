...
export default new Vuex.Store({
  ...
  actions: {
    ...
    /* Watchlist (A mettre dans un module) */
    async addMovieWatchlist(context, payload) {
      let options = {
        method: "POST",
        body: JSON.stringify({
          media_type: "movie",
      ...
      return fetch(
        `${APIConfig.apiUrl}/account/${payload.account_id}/watchlist?api_key=${APIConfig.apiKey}&session_id=${payload.session_id}`,
        options
      )
        .then((response) => response.json())
        .then((json) => {
    ...
    async deleteMovieWatchlist(context, payload) {
      let options = {
        method: "POST",
        body: JSON.stringify({
          media_type: "movie",
      ...
      return fetch(
        ...
        .then((json) => {
          ...
          return {
            code: 200,
            message: "Film supprimé de la watchlist avec succés !",
          };
        })
        .catch((err) => console.error(err));
    },
    async getWatchlistMovie(context, payload) {
      return fetch(
        `${APIConfig.apiUrl}/account/${payload.account_id}/watchlist/movies?api_key=${APIConfig.apiKey}&language=fr-FR&session_id=${payload.session_id}`
      )
        .then((response) => response.json())
        .then((json) => {
